// tasks/task_01_providers/report.js
...
export function enrichProviders(providers) {
  return providers.map(p => {
    const staleness = checkStaleness(p.lastProcessedTime);
    return { ...p, _staleness: staleness };
  });
}

export function buildReport(region, enrichedProviders) {
  ...
  if (criticals.length > 0) {
    ...
    for (const p of criticals) {
      lines.push(`  • ${p.accountName} (${p.cloudProvider})`);
  ...
  if (ok.length > 0) {
    lines.push(`  ── OK ────────────────────────────────────────────`);
    for (const p of ok) {
      lines.push(`  • ${p.accountName} (${p.cloudProvider}) — ${p._staleness.label}`);
    }
    lines.push(``);
  }
